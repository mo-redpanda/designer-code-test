...
export const MiniCalendarNavigation = ({
  ...
  if (asChild) {
    return (
      <Slot.Root onClick={handleClick} data-testid={testId} {...props}>
        {children}
      </Slot.Root>
    );
  }
...
export const MiniCalendarDays = ({ className, children, testId, ...props }: MiniCalendarDaysProps) => {
  const { startDate, days: dayCount } = useMiniCalendar();
  const days = getDays(startDate, dayCount);
  return (
    <div className={cn('flex items-center gap-1', className)} data-testid={testId} {...props}>
...
export const MiniCalendarDay = ({ date, className, testId, ...props }: MiniCalendarDayProps) => {
  ...
  return (
    <Button
      className={cn(
        ...
        isSelected && 'bg-selected text-selected-foreground hover:bg-selected/90 hover:text-selected-foreground',
        className,
      )}
    ...
    >
      <span
        className={cn('font-medium text-[10px] text-muted-foreground', isSelected && 'text-selected-foreground/70')}
      >
        {month}
      </span>
      <span className={cn('font-semibold text-sm', isSelected && 'text-selected-foreground')}>{day}</span>
    </Button>
  );
};
